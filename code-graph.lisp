...
(defun code-graph(system
  ...
  (let*((graph(graph system))
	...
	(loop :for symbol :being :each :external-symbol :in (or package system)
	      :when (fboundp symbol)
	      :collect (make-dot symbol type(gethash symbol *codes*)direction))))))

(defun standalone-p(code)
  (typecase code
    (symbol (fboundp code))
    (code (null(code-commons code)))))

(defun privates(graph)
  (loop :for node :in graph
	:when(= 1 (count (car node) graph
