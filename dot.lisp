...
(defun external-symbolp(symbol)
  (eq :external(nth-value 1(find-symbol(symbol-name symbol)
			     (symbol-package symbol)))))

(defun symbol-names-file-p(symbol)
  (let((system(asdf:find-system(string-downcase(package-name(symbol-package symbol)))
		nil)))
    (when system
...
(defun edges(code)
  (append (loop :for common :in (code-commons code)
		:collect (or (gethash common *codes*)
			     common))
	  (unless *ignore-privates*
	    (loop :for private :in (code-privates code)
		  :collect (or (gethash private *codes*)
			       private)))))
